...
void Reassembler::extend( Datagram& datagram )
{
  ...
  for ( auto& datagram_ : datagram_vec ) {
    auto [first_index_, data_, is_last_substring_] = datagram_;
    const uint64_t last_index_ = first_index_ + data_.size() - 1;
    const bool is_overlap = ( first_index <= last_index_ ) && ( first_index_ <= last_index );
    if ( !is_overlap ) {
      datagram_queue.push( datagram_ );
    ...
    {
      data.insert( 0, data_.substr( 0, first_index - first_index_ ) ); // 10, 10-19, 15
      first_index = first_index_;
    }
    // deal with the post-part
    if ( last_index_ > last_index ) // extend data to the right
    {
      data += data_.substr( last_index + 1 - first_index_, last_index_ - last_index ); // 10, 10-19, 15
...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring, Writer& output )
{
  ...
  const bool out_of_range = first_index >= next_index + output.available_capacity(); // 10, 0-9, 10 is out of range
  if ( out_of_range ) {
    return; // just discard this datagram
  }

  // 10, 10-19, 15, 19-15+1=19+1-15=10+10-15
  const uint64_t in_range_size = min( data.size(), next_index + output.available_capacity() - first_index );
